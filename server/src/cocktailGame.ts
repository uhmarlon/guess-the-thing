import cocktail from './cocktail/cocktail.json';
import { Cocktail, Room, RoomGameMetadata } from './interfaces';
import { gameMeta, gameSetPersonString, gameSetRound, getPlayersInRoom, getPlayersPoints, io } from './index';
...
export async function gameCocktailLoop(roomName: string, rounds: number): Promise<void> {
...
export async function gameSetCocktails(roomName: string, clientDrinkInfo: { strDrink: string, idDrink: string }[]): Promise<void> {
    const room: Room = io.sockets.adapter.rooms.get(roomName) as unknown as Room
    if (!room) {
        return
    }
...
export async function gameSetCocktailIMG(roomName: string, strDrinkThumb: string): Promise<void> {
    const room: Room = io.sockets.adapter.rooms.get(roomName) as unknown as Room
    if (!room) {
        return
    }
